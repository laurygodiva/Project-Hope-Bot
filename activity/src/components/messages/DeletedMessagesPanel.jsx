...
export default function DeletedMessagesPanel() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/guild/message-logs/deleted?${params.toString()}`)
      .then(setEntries)
  ...
  return (
    <div className="send-message-page">
      ...
      {entries && entries.length === 0 && <p className="muted">No hay mensajes eliminados registrados.</p>}

      <div className="message-log-list">
        {entries?.map((e) => (
          <div key={`${e.messageId}-${e.deletedAt}`} className="gradient-frame">
            <div className="guide-box message-log-card">
              ...
              <p className="message-log-content">{e.content || <span className="muted">(sin contenido de texto)</span>}</p>

              <div className="message-log-row message-log-footer">
                <span>
                  Eliminado por:{' '}
                  <strong>{e.deletedBy?.tag || 'Desconocido (¿el propio autor o sin registro de auditoría?)'}</strong>
                </span>
                <span className="muted">{formatDate(e.deletedAt)}</span>
              </div>
            </div>
          </div>
        ))}
